...
class TestApproxTokenCount:
    ...
    def test_block_content_counts_only_input_text(self):
        """ブロック形式は input_text のみ数え、画像ブロックは無視する."""
        messages = [{
            'role': 'user',
            'content': [
                {'type': 'input_text', 'text': 'a' * 40},
                {'type': 'input_image', 'image_url': 'x' * 4000, 'detail': 'auto'},
            ],
        }]
        assert approx_token_count(messages) == 10
    ...
    def test_non_string_content_uses_str(self):
...
class TestTrimHistory:
    ...
    def test_over_limit_pops_oldest(self):
        """上限超過時は古いものから削除される."""
        history = [
            {'role': 'user', 'content': 'old' + 'a' * 400},
            {'role': 'assistant', 'content': 'mid' + 'b' * 400},
    ...
    def test_system_prompt_counts_toward_budget(self):
        ...
        history = [
            ...
            {'role': 'user', 'content': 'b' * 40},   # 10 tokens
        ]
...
class TestMakeContextEntry:
    ...
    def test_text_from_file_path(self, tmp_path):
    ...
    def test_images_become_blocks(self):
    ...
    def test_no_text_no_images_content_is_none(self):
    ...
    def test_images_only(self):
        ...
        assert entry['content'] == [
            {'type': 'input_image', 'image_url': urls[0], 'detail': 'auto'},
        ]
