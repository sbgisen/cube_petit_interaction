...
class TestSelectWithHysteresis:
    ...
    def test_first_selection_returns_best(self):
    ...
    def test_same_best_keeps_selection(self):
    ...
    def test_stays_on_last_when_diff_below_hysteresis(self):
    ...
    def test_switches_when_diff_equals_hysteresis(self):
        """差がちょうど閾値のとき (diff < hysteresis が偽) は切り替わる."""
        scores = {'idle': 0.5, 'approach': 0.65}  # diff = 0.15
        result = select_with_hysteresis(scores, 'idle', 100.0, 0.0, HYSTERESIS, HOLD_TIME)
        assert result == 'approach'

    def test_switches_when_diff_exceeds_hysteresis(self):
        """差が閾値を超え、ホールド時間も経過していれば切り替わる."""
        scores = {'idle': 0.3, 'approach': 0.9}
        result = select_with_hysteresis(scores, 'idle', 100.0, 0.0, HYSTERESIS, HOLD_TIME)
        assert result == 'approach'

    def test_hold_time_blocks_switch(self):
    ...
    def test_zero_hysteresis_zero_hold(self):
...
class TestAddRandomNoise:
    ...
    def test_all_keys_modified_in_place(self):
    ...
    def test_default_rng_within_bounds(self):
